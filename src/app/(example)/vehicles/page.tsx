...
export default async function VehiclesItemPage({
  ...
  const response = await fetch(`https://swapi.dev/api/vehicles${search ? `?search=${search}` : ""}`);
  ...
  return (
    <div>
      <div style={{ color: "blue", fontSize: "60px" }}>Vehicles</div>
      {categoryData.map((item: SwapiListItem) => (
        <div key={item.url}>
          <Link href={`/vehicles/${slugify(item.name ? item.name : item.title || "")}`}>
